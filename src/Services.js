const services = [
  {
    id: 1,
    name: "Hair",
    h2: "Our Signature Services",
    subt: "Hair Care That Turns Heads",
    services: [
      {
        title: "Haircut & Styling",
        desc: "Precision cuts, flawless blowouts, and stunning event styling.",
      },
      {
        title: "Color Creations",
        desc: "From subtle highlights to bold balayage — color that defines you.From subtle highlights to bold balayage — color that defines you.",
      },
      {
        title: "Hair Treatments",
        desc: "Restore shine, strengthen strands, and nourish your scalp.",
      },
    ],
    img: {
      src: "src/assets/images/Hair img section.jpg",
      alt: "Hair service image",
    },
  },
  {
    id: 2,
    name: "Nails",
    h2: "Nail Perfection",
    subt: "Manicures & Pedicures You’ll Love",
    services: [
      {
        title: "Manicures",
        desc: "Classic, gel, French, and spa pampering for flawless hands.",
      },
      {
        title: "Pedicures",
        desc: "Relaxing foot care with luxurious treatments and callus removal.",
      },
      {
        title: "Nail Art & Enhancements",
        desc: "Custom designs, acrylics, and gel extensions — express your style.",
      },
    ],
    img: {
      src: "src/assets/images/Nails img section.jpg",
      alt: "Nails service image",
    },
  },
  {
    id: 3,
    name: "Skincare",
    h2: "Nail Perfection",
    subt: "Manicures & Pedicures You’ll Love",
    services: [
      {
        title: "Manicures",
        desc: "Classic, gel, French, and spa pampering for flawless hands.",
      },
      {
        title: "Pedicures",
        desc: "Relaxing foot care with luxurious treatments and callus removal.",
      },
      {
        title: "Nail Art & Enhancements",
        desc: "Custom designs, acrylics, and gel extensions — express your style.",
      },
    ],
    img: {
      src: "src/assets/images/Skincare img section.jpg",
      alt: "Nails service image",
    },
  },
  {
    id: 4,
    name: "Waxing",
    h2: "Silky Smooth Confidence",
    subt: "Waxing & Hair Removal",
    services: [
      {
        title: "Waxing",
        desc: "Classic, gel, French, and spa pampering for flawless hands.",
      },
    ],
    img: {
      src: "src/assets/images/Waxing img section.jpg",
      alt: "Waxing service image",
    },
  },
  {
    id: 5,
    name: "Massage",
    h2: "Relax & Rejuvenate",
    subt: "Massage Therapy",
    services: [
      {
        title: "Massage",
        desc: "Swedish, deep tissue, hot stone & more — tailored to melt stress and restore balance.",
      },
    ],
    img: {
      src: "src/assets/images/Massage img section.jpg",
      alt: "Massage service image",
    },
  },
];

export default services;
